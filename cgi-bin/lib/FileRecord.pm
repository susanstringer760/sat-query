...
sub getHtmlRow 
{
	...
	my $str = "<tr bgcolor=$color>\n";
                #$str = $str . "<td><input type=\"checkbox\" name=\"mss_files\" value=\"$self->{size};$self->{path}\" onClick=\"check_size($index);\"></td>";
		...
		$path =~ s/^_//g;
		$value = "$path;$self->{size}";
                $str = $str . "<td><input type='checkbox' name='data_files' value='$value' onClick=\"add_file('$value')\"/td>";
...
sub getTableHeader
{
	...
	$str = $str . "<tr bgcolor=#6600FF>\n";
		$str = $str . "<td nowrap align=center><font size=-1 color=white><b>Data<br>Retrieval</b></td>\n";
